...
public class MapGenerator : MonoBehaviour
{
    ...
    void SetupDungeon()
    {
        ...
        if (RoomTransitionManager.instance != null)
            RoomTransitionManager.instance.ClearRooms();
    ...
    void UpdateSpecialRoomVisuals()
    {
        foreach (var cell in spawnedCells)
        {
            ...
            if (cell.index == itemRoomIndex)   { icon = item;   cell.SetRoomType(RoomType.Item); }
            if (cell.index == shopRoomIndex)   { icon = shop;   cell.SetRoomType(RoomType.Shop); }
            if (cell.index == bossRoomIndex)   { icon = boss;   cell.SetRoomType(RoomType.Boss); }
            if (cell.index == secretRoomIndex) { icon = secret; cell.SetRoomType(RoomType.Secret); }

            if (icon != null)
            {
                cell.SetSpecialRoomSprite(icon);
                cell.spriteRenderer.enabled = true; // ikonu göster
            }
        }
    }
    ...
    private void SpawnRoom(int index)
    {
        ...
        // Special Room Icon renderer'ını başta gizle
        if (newCell.spriteRenderer != null)
            newCell.spriteRenderer.enabled = false;
