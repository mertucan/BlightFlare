...
public class RoomEnemyTracker : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        foreach (var heart in GetComponentsInChildren<HeartAI>(true))
            enemies.Add(heart.gameObject);
        foreach (var mask in GetComponentsInChildren<MaskAI>(true))
            enemies.Add(mask.gameObject);
