...
public class Room : MonoBehaviour
{
    ...
    public void SetupRoom(Cell currentCell, RoomScriptable room)
    {
        ...
        if (currentCell.roomType != RoomType.Secret)
        {
            ...
            {
                case RoomShape.OneByOne: SetupOneByOne(currentCell, floorplan, cellList); break;
                case RoomShape.OneByTwo: SetupOneByTwo(currentCell, floorplan, cellList); break;
                case RoomShape.TwoByOne: SetupTwoByOne(currentCell, floorplan, cellList); break;
                case RoomShape.TwoByTwo: SetupTwoByTwo(currentCell, floorplan, cellList); break;
                case RoomShape.LShape:   SetupLShapeRoom(currentCell, floorplan, cellList); break;
            }
        }

        GenerateWalls(currentCell.roomShape, currentCell);
        SetupSecretRoomWalls(currentCell, floorplan, cellList);
    ...
    private void SetupSecretRoomWalls(Cell currentCell, int[] floorplan, List<Cell> cellList)
    {
        var directions = new[]
        {
            EdgeDirection.Up,
        ...
        Debug.Log($"[Room] SetupSecretRoomWalls başladı → oda:{gameObject.name}, " +
                  $"roomType:{currentCell.roomType}, cellList:[{string.Join(",", currentCell.cellList)}]");

        foreach (int cellIdx in currentCell.cellList)
        {
            foreach (var dir in directions)
                TryAddSecretWall(cellIdx, dir, floorplan, cellList, currentCell.roomType);
        }
    }

    private void TryAddSecretWall(int fromIndex, EdgeDirection dir,
        int[] floorplan, List<Cell> cellList, RoomType thisRoomType)
    {
        int neighbourIndex = fromIndex + GetOffset(dir);
        if (neighbourIndex < 0 || neighbourIndex >= floorplan.Length) return;
        if (floorplan[neighbourIndex] != 1) return;

        var foundCell = cellList.FirstOrDefault(c => c.cellList.Contains(neighbourIndex));
        if (foundCell == null) return;

        // DURUM A: Normal oda → Gizli oda
        if (thisRoomType != RoomType.Secret && foundCell.roomType == RoomType.Secret)
        {
            Debug.Log($"[Room] TryAddSecretWall: Normal→Gizli. fromIdx:{fromIndex}, dir:{dir}, secretIdx:{neighbourIndex}");
            var srw = PlaceSecretRoomWall(dir, neighbourIndex);
            srw.pairedCellIndex = fromIndex;
            return;
        }

        // DURUM B: Gizli oda → Normal oda
        if (thisRoomType == RoomType.Secret && foundCell.roomType != RoomType.Secret)
        {
            Debug.Log($"[Room] TryAddSecretWall: Gizli→Normal. fromIdx:{fromIndex}, dir:{dir}, normalIdx:{neighbourIndex}");
            var srw = PlaceSecretRoomWall(dir, neighbourIndex);
            srw.pairedCellIndex = fromIndex;
        }
    }

    private SecretRoomWall PlaceSecretRoomWall(EdgeDirection dir, int neighbourIndex)
    {
        var half = RoomManager.instance.RoomInnerHalfSize;

        Vector3 localPos = dir switch
        {
            EdgeDirection.Up    => new Vector3( 0f,     half.y, 0f),
            EdgeDirection.Down  => new Vector3( 0f,    -half.y, 0f),
            EdgeDirection.Left  => new Vector3(-half.x,  0f,   0f),
            EdgeDirection.Right => new Vector3( half.x,  0f,   0f),
            _                   => Vector3.zero
        };

        Vector2 colSize = (dir == EdgeDirection.Up || dir == EdgeDirection.Down)
            ? new Vector2(DoorGapHalf * 2f, WallThickness * 2f)
            : new Vector2(WallThickness * 2f, DoorGapHalf * 2f);

        var wallGO = new GameObject($"SecretWall_{dir}_{neighbourIndex}");
        ...
        var srw = wallGO.AddComponent<SecretRoomWall>();
        srw.Setup(colSize);

        var door = wallGO.AddComponent<Door>();
        door.spriteRenderer = sr;
        door.SetupTransition(dir, neighbourIndex);
        door.targetRoomType = RoomType.Secret;
        door.SetLocked(true, null);
        door.SetDoorSprite(null);

        // Door'un eklediği fazla collider'ları temizle, sadece srw'nin _solidCol kalsın
        var allCols = wallGO.GetComponents<BoxCollider2D>();
        for (int i = 1; i < allCols.Length; i++)
        {
            Debug.Log($"[Room] Fazla collider silindi → size:{allCols[i].size}, isTrigger:{allCols[i].isTrigger}");
            Destroy(allCols[i]);
        }

        srw.linkedDoor      = door;
        srw.secretCellIndex = neighbourIndex;
        srw.wallDirection   = dir;

        srw.openedDoorPrefab =
            RoomManager.instance.secretOpenedDoorPrefab != null
                ? RoomManager.instance.secretOpenedDoorPrefab
                : RoomManager.instance.openedShopDoorPrefab;

        if (srw.openedDoorPrefab == null)
            Debug.LogError($"[Room] openedDoorPrefab NULL! RoomManager Inspector'da secretOpenedDoorPrefab atayın.");

        if (RoomManager.instance.secretExplosionClips != null &&
            RoomManager.instance.secretExplosionClips.Length > 0)
        {
            srw.explosionClips    = RoomManager.instance.secretExplosionClips;
            srw.selectedClipIndex = 0;
        }

        Debug.Log($"[Room] ✅ SecretWall kuruldu → dir:{dir}, idx:{neighbourIndex}, " +
                  $"worldPos:{wallGO.transform.position}, colSize:{colSize}");
    ...
    private void InstantiateRoomDesign(RoomScriptable room, RoomShape shape)
    {
        ...
        else
        {
            var fallback = System.Array.Find(RoomManager.instance.rooms,
                r => r != null && r.roomShape == shape &&
                     r.roomDesignPrefabs != null && r.roomDesignPrefabs.Length > 0);
            if (fallback != null)
    ...
    private const float DoorInset = 0.2f;
    ...
    private EdgeDirection GetOppositeDirection(EdgeDirection dir) => dir switch
    {
    ...
    public void SetupLShapeRoom(Cell cell, int[] floorplan, List<Cell> cellList)
    {
        ...
        else if (cellA + 1 == cellB && cellB + 10 == cellC)
        {
            ...
            TryPlaceDoor(cellC, new Vector2( 1f,     -2.6125f), EdgeDirection.Left,  floorplan, cellList, cell);
        }
        else if (cellA + 10 == cellB)
        {
            ...
            TryPlaceDoor(cellC, new Vector2( 5.3125f, -1f),     EdgeDirection.Up,    floorplan, cellList, cell);
            TryPlaceDoor(cellC, new Vector2( 5.3125f, -4.5f),   EdgeDirection.Down,  floorplan, cellList, cell);
            TryPlaceDoor(cellC, new Vector2( 9.75f,  -2.6375f), EdgeDirection.Right, floorplan, cellList, cell);
        }
        ...
        {
            TryPlaceDoor(cellA, new Vector2( 5.3125f,  4.5f),   EdgeDirection.Up,    floorplan, cellList, cell);
            TryPlaceDoor(cellA, new Vector2( 1f,      2.6125f), EdgeDirection.Left,  floorplan, cellList, cell);
    ...
    private int GetOffset(EdgeDirection direction) => direction switch
    {
        EdgeDirection.Up    => -10,
        EdgeDirection.Down  =>  10,
        EdgeDirection.Right =>   1,
        EdgeDirection.Left  =>  -1,
        _                   =>   0
    };
    ...
    private void GenerateLShapeInnerWalls(Cell cell, Vector2 halfSize)
    {
        ...
        if      (cellA + 1 == cellB && cellA + 10 == cellC) { GenerateEdgeWall(true,  0f,  0f,         halfSize.x, EdgeDirection.Down);  GenerateEdgeWall(false, 0f, -halfSize.y, 0f, EdgeDirection.Right); }
        else if (cellA + 1 == cellB && cellB + 10 == cellC) { GenerateEdgeWall(true,  0f, -halfSize.x,  0f,        EdgeDirection.Down);  GenerateEdgeWall(false, 0f, -halfSize.y, 0f, EdgeDirection.Left);  }
        else if (cellA + 10 == cellB)                        { GenerateEdgeWall(true,  0f,  0f,         halfSize.x, EdgeDirection.Up);    GenerateEdgeWall(false, 0f,  0f, halfSize.y, EdgeDirection.Right); }
        else if (cellA + 10 == cellC)                        { GenerateEdgeWall(true,  0f, -halfSize.x,  0f,        EdgeDirection.Up);    GenerateEdgeWall(false, 0f,  0f, halfSize.y, EdgeDirection.Left);  }
    }
    ...
    private void CreateWallCollider(bool isHorizontal, float perpPos, float start, float end)
    {
        ...
        if (isHorizontal)
        {
            ...
            wall.AddComponent<BoxCollider2D>().size = new Vector2(length, WallThickness);
        }
        else
        {
            wall.transform.localPosition = new Vector3(perpPos, mid, 0);
            wall.AddComponent<BoxCollider2D>().size = new Vector2(WallThickness, length);
        }
    }
